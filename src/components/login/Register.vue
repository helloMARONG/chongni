<template>
  <div class="register">
    <app-header title="注册"></app-header>
    <div class="logo"><img src="static/images/logo.png" alt=""></div>
    <mu-form ref="form" :model="validateForm" class="login-content">
      <mu-form-item label="输入手机号" prop="username" :rules="usernameRules">
        <mu-text-field type="tel" v-model="validateForm.username" prop="username"></mu-text-field>
      </mu-form-item>
      <mu-form-item label="输入验证码" prop="smscode" :rules="smscodeRules">
        <sms-code v-model="validateForm.smscode" :phone="validateForm.username" @getCode="getCode" prop="smscode"></sms-code>
      </mu-form-item>
      <mu-form-item label="输入密码" prop="password" :rules="passwordRules">
        <mu-text-field type="password" v-model="validateForm.password" prop="password" help-text="密码为6-20位字母数字组合"></mu-text-field>
      </mu-form-item>
      <mu-form-item label="确认密码" prop="password2" :rules="password2Rules">
        <mu-text-field type="password" v-model="validateForm.password2" prop="password2"></mu-text-field>
      </mu-form-item>
      <mu-form-item label="邀请码" prop="invitecode" :rules="invitecodeRules">
        <mu-text-field v-model="validateForm.invitecode" prop="invitecode"></mu-text-field>
      </mu-form-item>
      <mu-form-item>
        <mu-button class="common-btn" @click="submit">注册</mu-button>
      </mu-form-item>
    </mu-form>
    <div class="login-bottom">
      已注册? <router-link to="/Login">登陆</router-link>
    </div>
  </div>
</template>

<script>
import smsCode from "@/components/bin/smsCode";
export default {
  name: "Login",
  components: {
    smsCode
  },
  data() {
    return {
      usernameRules: [
        { validate: val => !!val, message: "请输入手机号" },
        { validate: val => (/^[1][3-9][0-9]{9}$/.test(val)), message: "请输入正确的手机号" }
      ],
      passwordRules: [
        { validate: val => (/^(?=.*\d)(?=.*[a-zA-Z]).{6,20}$/.test(val)), message: "密码为6-20位字母数字组合"}
      ],
      password2Rules: [
        { validate: val => val == this.validateForm.password, message: "2次密码不一致"}
      ],
      smscodeRules: [{ validate: val => !!val, message: "请输入验证码" }],
      invitecodeRules: [{ validate: val => !!val, message: "请输入邀请码" }],
      validateForm: {
        username: "",
        password: "",
        password2: "",
        smscode: "",
        invitecode: ""
      }
    };
  },
  methods: {
    // 手机验证码
    getCode() {
      const formData = {
        mobile: this.validateForm.username
      }
      if(!(/^[1][3-9][0-9]{9}$/.test(this.validateForm.username))){
        this.$toast.message("请输入正确的手机号！");
      }else{
        this.$http.post("/api/index/sendSms", formData).then(response => {
          const res = response.data;
          this.$toast.message(res.msg);
        })
      }
    },
    submit() {
      this.$refs.form.validate().then(result => {
        if(result){
          const formData = {
            mobile: this.validateForm.username,
            password: this.validateForm.password,
            code: this.validateForm.smscode,
            sms: this.validateForm.invitecode
          };
          this.$http.post("/api/index/register", formData).then(response => {
            const res = response.data;
            if(res.status === 0){
              setTimeout(() => {
                this.$router.push('/Login');
              }, 1000);
            }
            this.$toast.message(res.msg);
          })
        }
      });
    }
  }
};
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
.register .mu-form-item{margin-bottom: 4px;}
.register .login-content{ padding-top: 20px;}
</style>
