<template>
  ...
  <div class="resetPassword">
    <app-header title="修改登陆密码"></app-header>
    <mu-form ref="form" :model="validateForm" class="form-content">
      <mu-form-item label="输入新密码" prop="newPassWord" :rules="newPassWordRules">
        <mu-text-field type="password" v-model="validateForm.newPassWord" prop="newPassWord"></mu-text-field>
      </mu-form-item>
      <mu-form-item label="确认新密码" prop="newPassWord2" :rules="newPassWord2Rules">
...
export default {
  ...
  data() {
    return {
      ...
      newPassWord2Rules: [
        { validate: val => !!val, message: "请输入确认密码" },
  ...
  methods: {
    ...
    getCode() {
      ...
      this.$http.post("/api/index/sendSms", formData).then(response => {
        ...
        if(res.status === 'y'){
          this.$toast.message('验证码已发送, 请注意查收!');
        }else{
    ...
    submit() {
      this.$refs.form.validate().then(result => {
        if (result) {
          const formData = {
            password: this.validateForm.newPassWord,
            sms_code: this.validateForm.smscode,
          };
          this.$http.post('/api/member/reset_password', formData)
          .then((response) => {
            console.log(response.data);
            const res = response.data;
            if (res.status === 'y') {
              this.$toast.message("修改密码成功");
...
<style scoped>
.resetPassword{ background: #F2F3F9; height: 100%; width: 100%; position: absolute;}
</style>
